...
public class SparseMatrix<anyType>{
	...
	public boolean add(int row, int col, anyType x) {
		...
		if(current.getNext()==null){
			...
			//(getIndex(current)<getIndex(row,col)) case should never happen 
		}
	...
	public int[] getLocation(anyType x) {
		...
		if(current==null)
			return null;
		if(current.getValue().equals(x)){
			location[0]=current.getY();
			location[1]=current.getX();
			return location;
		}
		while(current.getNext()!=null){
			if(current.getValue().equals(x)){
